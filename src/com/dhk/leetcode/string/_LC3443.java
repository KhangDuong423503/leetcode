...
// find one-pass alg
public class _LC3443 {
    public static void main(String[] args) {
        System.out.println(maxDistance("NWSE", 1) == 3);
        System.out.println(maxDistance("NSWWEW", 3) == 6);
    }

    private static int maxDistance(String s, int k) {
        int[] maxDistances = new int[] {
                maxDistancePlan(s, k, 'N', 'E'),
                maxDistancePlan(s, k, 'S', 'E'),
                maxDistancePlan(s, k, 'S', 'W'),
                maxDistancePlan(s, k, 'N', 'W')
        };
    ...
    private static int maxDistancePlan(String s, int k, char charToChangeNS, char charToChangeEW) {
        int currentDistance = 0;
        int maxDistance = 0;
        for (int i = 0; i < s.length(); i++) {
            char c = s.charAt(i);

            if (k > 0 || (c != charToChangeNS && c != charToChangeEW)) {
                currentDistance++;
