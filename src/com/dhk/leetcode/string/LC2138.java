...
// INSIGHTS: calculate size for result array to store substrings, pad with char if needed
public class LC2138 {
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(divideString("abcdefghijk", 3, 'x')));
    }

    private static String[] divideString(String s, int k, char fill) {
        // get size of return array from string size and k
        int numOfGroups = s.length() / k;
        if (s.length() % k != 0) {
            numOfGroups++;
        }
        String[] stringGroups = new String[numOfGroups];
        // loop multiples of k
        // get substring
        for (int i = 0; i < numOfGroups; i++) {
            String currentGroup;
            // if multiple of k is larger than string size, pad with fill char
            if (k * (i + 1) > s.length()) {
                StringBuilder builder;
                builder = new StringBuilder(s.substring(k * i));
                builder.append(String.valueOf(fill).repeat(k - (s.length() % k)));
                currentGroup = builder.toString();
            // otherwise, get substring normally
            } else {
                currentGroup = s.substring(k * i, k * (i + 1));
            }
            stringGroups[i] = currentGroup;
        }
...
the string, and so on. Each element can be a part of exactly one group.
